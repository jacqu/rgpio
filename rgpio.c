 ...
 * To use PWM, disable analog audio in /boot/config.txt
 * To enable PWM on GPIO 12 and 13, add this line in config.tx:
 * dtoverlay=pwm-2chan,pin=12,func=4,pin2=13,func2=4
 * To allow user PWM access on kernels > 4.16 add this udev rule
 * to /etc/udev/rules.d/99-com.rules:
SUBSYSTEM=="pwm*", PROGRAM="/bin/sh -c '\
        chown -R root:gpio /sys/class/pwm && chmod -R 770 /sys/class/pwm;\
        chown -R root:gpio /sys/devices/platform/soc/*.pwm/pwm/pwmchip* && chmod -R 770 /sys/devices/platform/soc/*.pwm/pwm/pwmchip*\
'"
 * On kernel < 4.16, add these lines to rc.local:
